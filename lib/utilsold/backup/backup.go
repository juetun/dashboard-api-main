...
func (bp *BackUpParam) FilePathIsNull() *BackUpParam {
	if bp.FilePath == "" {
		...
		bp.SetFilePath(conf2.BackUpFilePath)
	}
	return bp
}
...
func (bp *BackUpParam) FileNameIsNull() *BackUpParam {
	if bp.FileName == "" {
		zgh.ZLog().Warn("message", "data is null")
		bp.SetFileName(time.Now().Format("2006-01-02") + conf2.BackUpSqlFileName)
	}
	return bp
}
...
func (bp *BackUpParam) DurationIsNull() *BackUpParam {
	if bp.CronSpec == "" {
		zgh.ZLog().Warn("message", "data is null")
		bp.SetCronSpec(conf2.BackUpDuration)
	}
	return bp
}
...
func (bp *BackUpParam) doBackUp() {
	err := conn2.SqlDump(bp.FileName, bp.FilePath)
	if err != nil {

...
func (r *AsyncExport) SetExportData(args *wrappers.ArgumentsExportInit, model models.ZExportData) (p *AsyncExport) {
	r.model = model
	r.args = args
	return r
}
...
// 	return
// }
func (r *AsyncExport) doOtherPage(excel *ExcelOperate, page *Pager, artSheet *wrappers.ArgumentExportSheet) (err error) {
	var pageIndex = 1
...
func (r *AsyncExport) doOnePage(excel *ExcelOperate, artSheet *wrappers.ArgumentExportSheet) (pageData Pager, err error) {
	// 获取第一页数据
...
func (r *AsyncExport) getAccessUrl(argSheet *wrappers.ArgumentExportSheet) (res string, err error) {
	var serverMessage ServerConfig

	// 获取应用的服务器位置
	apps, err := dao_impl.NewDaoAppPath().GetAllApp()
	if err != nil {
		return
	}
...
// 获取数据
func (r *AsyncExport) getData(artSheet *wrappers.ArgumentExportSheet) (res Pager, err error) {
	res = Pager{}
